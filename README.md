...
專案提供了自動化建置腳本，位於 `scripts/` 目錄下：

### 腳本清單

| 腳本檔案 | 平台 | 說明 |
|---------|------|------|
| `build.bat` | Windows | Windows 批次檔，建置所有平台版本 |
| `build.sh` | Linux/macOS | Shell 腳本，建置所有平台版本 |

### 建置輸出

執行建置腳本後，會在 `dist/` 目錄下生成以下平台的執行檔：
```
dist/ 
├── win-x64/serupd.exe # Windows 64位元
├── win-x86/serupd.exe # Windows 32位元
├── linux-x64/serupd # Linux 64位元
├── osx-x64/serupd # macOS Intel
└── osx-arm64/serupd # macOS Apple Silicon
```
### 使用範例
...
# 使用建置的執行檔
dist\win-x64\serupd.exe C:\MyProject\Modules
```
**Linux/macOS:**
```bash
# 首次使用需設定權限
...
# 設定執行權限並使用
chmod +x dist/linux-x64/serupd dist/linux-x64/serupd /path/to/project/modules
```
## 使用方法
### 基本用法

serupd.exe <目錄路徑> [appsettings.json路徑]
...
1. 已安裝 .NET 9.0 SDK
2. Linux/macOS 使用者需先執行 `chmod +x scripts/build.sh`
3. 網路連線正常（需要下載 NuGet 套件）
